...
TEST_CASE("CDAL should be able to use all of its public methods correctly, allocating and deallocating nodes accordingly", "[CDAL]") {
COP3530::CDAL<int> list;

    SECTION("400 integers inserted using push_front") {
        for (int i = 0; i < 400; ++i)
            list.push_front(i);
        REQUIRE(list.size() == 400l);

        SECTION("Remove over half of the integers using pop_back; expect the empty nodes to be deleted") {
            for (int i = 0; i < 300; ++i)
                list.pop_back();
...
TEST_CASE("CDAL should throw exceptions every time, and only when, they are necessary", "[CDAL]") {
    COP3530::CDAL<int> list;
    ...
    SECTION("Require no throw on calling insert on one item past the end element") {
        ...
        SECTION("Require domain exception on calling insert on two items past the end element") {
    ...
    SECTION("Require domain error on calling item_at and subscript operators on nonexistent elements") {
        ...
        const COP3530::CDAL<int> constList = list;
        REQUIRE_THROWS_AS(constList[10], std::domain_error);
    }
}

TEST_CASE("Test to see if the iterator for CDAL works properly and throws the correct exceptions", "[CDAL]") {
    COP3530::CDAL<int> list;
    COP3530::CDAL<int>::iterator iter = list.begin();
    for (int i = 0; i < 20; ++i)
        list.push_back(i);
    const COP3530::CDAL<int> constList = list;
    COP3530::CDAL<int>::const_iterator constIter = constList.begin();
    ...
    SECTION("Check to see if each iterator can traverse the entire list and spit out correct values") {
        ...
        SECTION("See if appropriate errors are thrown in both types") {
            ...
            SECTION("Also check to see if the structure dereference is working properly and throws appropriate errors") {
                COP3530::CDAL<TestNode> structList;
                structList.push_back(*(new TestNode()));
                const COP3530::CDAL<TestNode> constStructList = structList;
                COP3530::CDAL<TestNode>::iterator strIter = structList.begin();
                COP3530::CDAL<TestNode>::const_iterator constStrIter = constStructList.begin();
...
TEST_CASE("SDAL should be able to use all of its public methods correctly", "[SDAL]") {
...
TEST_CASE("SDAL should throw exceptions every time, and only when, they are necessary", "[SDAL]") {
    COP3530::SDAL<int> list;
    ...
    SECTION("Require domain error on calling item_at and subscript operators on nonexistent elements") {
        REQUIRE_THROWS_AS(list.item_at(10), std::domain_error);
        REQUIRE_THROWS_AS(list[10], std::domain_error);
        const COP3530::SDAL<int> constList = list;
        REQUIRE_THROWS_AS(constList[10], std::domain_error);
    }
}

TEST_CASE("Test to see if the iterator for SDAL works properly and throws the correct exceptions", "[SDAL]") {
    COP3530::SDAL<int> list;
    COP3530::SDAL<int>::iterator iter = list.begin();
    for (int i = 0; i < 20; ++i)
        list.push_back(i);
    const COP3530::SDAL<int> constList = list;
    COP3530::SDAL<int>::const_iterator constIter = constList.begin();
    ...
    SECTION("Check to see if each iterator can traverse the entire list and spit out correct values") {
        ...
        SECTION("See if appropriate errors are thrown in both types") {
            ...
            SECTION("Also check to see if the member operator is working properly and throws appropriate errors") {
                COP3530::SDAL<TestNode> structList;
                structList.push_back(*(new TestNode()));
                const COP3530::SDAL<TestNode> constStructList = structList;
                COP3530::SDAL<TestNode>::iterator strIter = structList.begin();
                COP3530::SDAL<TestNode>::const_iterator constStrIter = constStructList.begin();
                REQUIRE(strIter->data == 1);
                REQUIRE(constStrIter->data == 1);
                ++strIter;
                ++constStrIter;
...
TEST_CASE("PSLL should be able to use all of its public methods correctly", "[PSLL]") {
...
TEST_CASE("PSLL should throw exceptions every time, and only when, they are necessary", "[PSLL]") {
    COP3530::PSLL<int> list;
    ...
    SECTION("Require domain error on calling item_at and subscript operators on nonexistent elements") {
        REQUIRE_THROWS_AS(list.item_at(10), std::domain_error);
        REQUIRE_THROWS_AS(list[10], std::domain_error);
        const COP3530::PSLL<int> constList = list;
        REQUIRE_THROWS_AS(constList[10], std::domain_error);
    }
}

TEST_CASE("Test to see if the iterators for PSLL works properly and throws the correct exceptions", "[PSLL]") {
    COP3530::PSLL<int> list;
    COP3530::PSLL<int>::iterator iter = list.begin();
    for (int i = 0; i < 20; ++i)
        list.push_back(i);
    const COP3530::PSLL<int> constList = list;
    COP3530::PSLL<int>::const_iterator constIter = constList.begin();
    ...
    SECTION("Check to see if each iterator can traverse the entire list and spit out correct values") {
        ...
        SECTION("See if appropriate errors are thrown in both types") {
            ...
            SECTION("Also check to see if the structure dereference is working properly and throws appropriate errors") {
                COP3530::PSLL<TestNode> structList;
                structList.push_back(*(new TestNode()));
                const COP3530::PSLL<TestNode> constStructList = structList;
                COP3530::PSLL<TestNode>::iterator strIter = structList.begin();
                COP3530::PSLL<TestNode>::const_iterator constStrIter = constStructList.begin();
                REQUIRE(strIter->data == 1);
                REQUIRE(constStrIter->data == 1);
                ++strIter;
                ++constStrIter;
